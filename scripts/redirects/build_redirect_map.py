...
def write_readme(rows, nextjs_redirects, gone, product_categories, product_tags,
    ...
    a("### Trailing slash")
    a("")
    a("Every legacy URL ends in `/`. Next.js normalizes that away with its own 308")
    a("**before** matching `redirects()`, and a `source` written with a trailing slash is")
    a("unreachable (verified against a running server: it 404s without the slash and only")
    a("ever 308s to the slash-free form). Sources are therefore stored slash-free, which")
    a("is the only form that matches. The consequence is that a legacy `/path/` request")
    a("costs one platform normalization hop before the redirect. That is not a chain in")
    a("the map - `verification-report.md` counts it separately - but to land legacy links")
    a("in a genuine single hop, strip the trailing slash at the CDN/edge before Next.js")
    a("sees the request.")
    a("")
    a("### Query parameters")
    a("")
    ...
    a("SHOPIFY_OPTIONAL=1 NODE_ENV=production npx next start -p 3120 &")
    a("")
    a("# 3. Verify every row against the running server")
    a("python3 scripts/redirects/verify_redirects.py --base-url http://localhost:3120")
    a("")
    a("# 4. Stop the server")
    a("kill %1")
    a("```")
    ...
    a(f"- `gone.json` ({len(gone)} paths) is not wired up yet. A future route handler (or")
    a("  proxy) should read it and answer 410 with a retired-page body. Until then those")
    a("  paths fall through to the 404 page, which is the correct user experience but the")
    a("  wrong status code for crawlers.")
